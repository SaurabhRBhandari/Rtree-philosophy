...
struct RStarBoundingBox
{
	...
	// maintain no. of objects, sum, and sum of squares for each dimension
	int n[classes];
	double sum[dimensions][classes];
	double sum_sq[dimensions];
	...
	bool stretch(const RStarBoundingBox<dimensions> &bb)
	{
		...
		for (int i = 0; i < classes; i++)
		{
			...
			for (std::size_t axis = 0; axis < dimensions; axis++)
			{
				sum[axis][i] += bb.sum[axis][i];
			}
		}
		for (std::size_t axis = 0; axis < dimensions; axis++)
		{
			sum_sq[axis] += bb.sum_sq[axis];
		}
		return ret;
	}

	inline int noOfObjects() const
	{
		return n;
	}

	inline double getSum(int axis) const
	{
		return sum[axis];
	}

	inline double getSumSq(int axis) const
	{
		return sum_sq[axis];
	}
		...
		for (int c = 0; c < classes; c++)
		{
			...
			for (std::size_t axis = 0; axis < dimensions; axis++)
			{
				if (edges[axis].first != bb.edges[axis].first || edges[axis].second != bb.edges[axis].second)
					return false;
				if (sum[axis][c] != bb.sum[axis][c])
					return false;
				if (sum_sq[axis] != bb.sum_sq[axis])
					return false;
			}
		}
		return true;
