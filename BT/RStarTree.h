...
public:
	...
	void preOrderTraversal(Node *node)
	{
		if (node == NULL)
			return;
		green << "Node: " << node->bound.ToString() << endl;
		green << "Items: " << node->items.size() << endl;
		for (int i = 0; i < node->items.size(); i++)
		{
			if (node->hasLeaves)
				yellow << "Leaf: " << static_cast<Leaf *>(node->items[i])->bound.ToString() << endl;
			else
				preOrderTraversal(static_cast<Node *>(node->items[i]));
		}
	}

	vector<BoundingBox> getFrontier(int n)
	{
		...
		while (no_of_visited_nodes < n && !q.empty())
		{
			...
			{
				if (node->hasLeaves)
					frontier.push_back(static_cast<Leaf *>(node->items[i])->bound);
				else
	...
	*/
	...
	struct QueryFunctor
	{
		...
		void operator()(BoundedItem *item)
		{
			...
			{
				blue << (node->items[0]->bound).ToString() << endl;
				if (node->hasLeaves)
